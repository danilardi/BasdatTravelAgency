    ...
    <div class="container">
        ...
        <div class="content">
            <table class="table1">
                ...
                $query = pg_query("SELECT * FROM pesanan JOIN paket ON pesanan.id_paket = paket.id JOIN transportasi ON pesanan.id_transportasi = transportasi.id WHERE id_pengguna = $id_pengguna ORDER BY pesanan.id ASC;");
                // $query = mysqli_query($db, $sql);
                $no = 1;
                $query1 = pg_query("SELECT id FROM pesanan WHERE id_pengguna = $id_pengguna ORDER BY id;");
                ...
                while ($pesanan = pg_fetch_array($query) and $edit = pg_fetch_array($query1)) {
                    ...
                    echo "<td>" . $pesanan['nomor_hp'] . "</td>";
                    echo "<td>" . $pesanan['tanggal_pemesanan'] . "</td>";
                    echo "<td>";
                    echo "<a href='prosesHapusPesanan.php?id=" . $edit['id'] . "'>Hapus</a>";
                    echo "<br>";
                    echo "<a href='editpesanan.php?id=" . $edit['id'] . "'>Edit</a>";
                    echo "</td>";
            ...
            <a href="visitor.php" id="back-btn"> Back</a>
        </div>
    </div>
