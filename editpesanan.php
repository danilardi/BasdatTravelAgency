...
if (isset($_POST['edit'])) {
    ...
    // buat query
    $query = pg_query("UPDATE pesanan SET id_pengguna = $id_pengguna, id_paket = $id_paket, id_transportasi = $id_transportasi, jumlah_orang = $jumlah_pemesan, durasi = $durasi, nomor_hp = '$nomor_hp', usia = $usia, alamat = '$alamat' WHERE id = $id;");

    // apakah query simpan berhasil?
    if ($query == TRUE) {
        header('Location: daftarpesanan.php?status=sukses');
    } else {
        header('Location: daftarpesanan.php?status=gagal');
    }
} ?>